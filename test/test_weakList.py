import pytest
from jsonc_sdict.weakList import Value, WeakList


class TestWeakListBasic:
    """基础功能测试"""
    ...
    def test_weak_reference_behavior(self):
        ...
        assert wl.tuple == (v1, v2), f"初始化去重失败，预期 (t0, t1)，实际 {wl}"
        del v1  # 删除外部引用，弱引用失效
        assert wl.tuple == (v2,), f"弱引用失效失败，预期 (t1,)，实际 {wl}"
...
class TestWeakListInsert:
    ...
    def test_insert_no_repeat(self):
        ...
        wl_no_repeat.insert(1, v1)  # 插入重复项，自动去重
        assert wl_no_repeat.tuple == (
            v1,
            v2,
        ), f"noRepeat insert去重失败，预期 (v1,v2)，实际 {wl_no_repeat.tuple}"
...
class TestWeakListRemoveAndDelItem:
...
class TestWeakListGetSetItem:
    ...
    def test_setitem_no_repeat(self):
        ...
        v2 = Value(2)
        v7 = Value(7)
        wl_no_repeat = WeakList([v1, v7], noRepeat=True)
        wl_no_repeat[1] = v1  # 替换为已存在值，自动去重
        assert wl_no_repeat.tuple == (
            v1,
        ), f"__setitem__去重错误，预期 (v1,)，实际 {wl_no_repeat.tuple}"


class TestWeakListNoRepeat:
    """noRepeat功能测试"""
...
class TestWeakListSortAndReverse:
    """sort和reverse方法测试"""
...
class TestWeakListMiscMethods:
    ...
    def test_contains(self):
        ...
        assert (
            Value(8) not in wl_contains
        ), f"__contains__错误，预期 Value(8) 不在列表中"


class TestWeakListOperators:
    """运算符测试（+, +=, *, *=）"""
